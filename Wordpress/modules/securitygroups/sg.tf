...
resource "aws_security_group" "alb_sg" {
  ...
  dynamic "ingress" {
    ...
    content {
      ...
      cidr_blocks = length(lookup(ingress.value, "cidr_blocks", [])) > 0 ? lookup(ingress.value, "cidr_blocks", []) : ["0.0.0.0/0"]
    }
  }
 ...
 dynamic "egress" {
    ...
    content {
      ...
      cidr_blocks = length(lookup(egress.value, "cidr_blocks", [])) > 0 ? lookup(egress.value, "cidr_blocks", []) : ["0.0.0.0/0"]
    }
  }
}
